...
public class P_11_20 {
	...
	<평균> 	 
		...
		System.out.println("=====================================================");
		for (int i = 0 ; i <args.length ; i++) {
			if (i % 5 == 0) { // 5로 나누 없을 때 0일때 개행, 0일 때는 제외
				if (i != 0) {
					System.out.println();
				}
			}
		...
		for (int i = 0 ; i < args.length; i++) {
			...
			if (i == 4 || i == 9 || i == 14 || i == 19) {
				scienceSum += Integer.parseInt(args[i]);
				}
		...
		engAvg = engSum / 4.0 ;
		mathAvg = mathSum / 4.0 ;
		scienceAvg = scienceSum / 4.0 ;
		
		allAvg = allSum / 16.0 ;

		System.out.print("<합계> : ");
		System.out.print(korSum + "\t" + engSum + "\t" + mathSum + "\t" + scienceSum + "\t" + allSum + "\t" + allAvg + "\n");
		System.out.print("<평균> : ");
		System.out.print(korAvg + "\t" + engAvg + "\t" + mathAvg + "\t" + scienceAvg + "\n");
